...
- **Экспорт** в один файл с перекодированием, прогрессом и отменой.
...
Все клипы приводятся к общему профилю из `settings.json` и склеиваются фильтром `concat`. Прогресс читается из лога ffmpeg, `Cancel` мягко останавливает процесс и удаляет недописанный файл.
...
Результат публикации — папка поставки: `VideoTinyTool.exe`, рядом `ffmpeg.exe`, `ffprobe.exe` и `fonts/`. Все DLL, включая нативные CSFML, лежат внутри exe и выкладываются в `runtime/` рядом с ним при первом запуске. `settings.json` создаётся там же.
...
Создаётся при первом запуске рядом с exe, читается один раз при старте, приложением никогда не перезаписывается. Битый файл не роняет приложение — недостающие поля берутся из значений по умолчанию.
...
Юнит-тесты покрывают доменную модель, команды с откатом, разбор JSON `ffprobe`, разбор строк лога ffmpeg и сборку строки аргументов экспорта. ffmpeg при этом не запускается, тестов интерфейса нет.

## Вне объёма этой версии

Переходы между клипами, несколько дорожек, отдельная работа со звуком, drag & drop файлов в окно, сохранение проекта, экспорт без перекодирования, экран настроек, локализация, другие ОС.

...
public class OverlayTransformTests
{
    ...
    {
        var clamped = new OverlayTransform(-1f, 2f, 0f, 5f).Clamped();
    ...
    {
        Assert.Equal(1f, new OverlayTransform(0f, 0f, 4f, 1f).Clamped().Width);
    }
    ...
    public void SetOverlay_StoresTheClampedTransform()
    {
        ...
        timeline.SetOverlay(clip, new OverlayTransform(0.5f, 0.5f, 2f, 0.5f));

        Assert.Equal(new OverlayTransform(0.5f, 0.5f, 1f, 0.5f), clip.Overlay);
    }

    [Fact]
    public void WithBounds_CarriesGapTransformAndAudio()
    {
        var timeline = new Timeline();
        var clip = TestData.Clip(Guid.NewGuid(), 0, 4);
        timeline.Add(clip);
        timeline.SetLeadingGap(clip, TimeSpan.FromSeconds(3));
        timeline.SetOverlay(clip, new OverlayTransform(0.1f, 0.2f, 0.3f, 0.4f));
        timeline.SetClipAudio(clip, new ClipAudio(0.5f, true));
        ...
        Assert.Equal(clip.Audio, copy.Audio);
    }
}
